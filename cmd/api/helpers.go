...
// then convert it to an integer and return it. If the operation isnt
// successfull, return 0 and an error.
...
// writeJSON() helper for sending responses. This takes a destination
// http.ResponseWriter, the HTTP status code to send, the data to encode
// to JSON, and a header map containing any additional HTTP headers that
...
func (app *application) writeJSON(w http.ResponseWriter, status int, data envelope, headers http.Header) error {
	...
	// Loop through header map and add each header to the http.ResponseWriter
	// header map.
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst interface{}) error {
	...
	// Initialize the json.Decoder, and call the DisallowUnkownFields() method
	// on it before decoding.
	...
	if err != nil {
		...
		// type *json.SyntaxError. If it does, then return a plain-english
		// error message which includes the location of the problem.
	...
	// is additional data in the request body and we return a custom error message.
...
// splits it into a slice on the comma character. If no matching could be found
...
// The readInt() helper reads a string value from the query string and converts it
// to an integer before returning. If no matching key could be found it returns the
// provided default value. If the value couldn't be converted to an integer, then
